...
def process_tile(tile_path):
    ...
    if mode is not None:
        return [(scale, tile, mode, rel_freq) for scale in RESIZING_SCALES]
    return []


def process_path(path):
    if os.path.isdir(path):
        tile_paths = [os.path.join(path, tile_name) for tile_name in os.listdir(path)]
        return [process_tile(tile_path) for tile_path in tile_paths]
    return []
...
def load_tiles(paths):
    ...
    with Pool(POOL_SIZE) as pool:
        all_results = []
        for path in paths:
            results = pool.apply(process_path, (path,))
            all_results.extend(results)

    for results in all_results:
        for scale, tile, mode, rel_freq in results:
...
def get_processed_image_boxes(image_path, tiles):
    ...
    img = read_image(
        image_path, mainImage=True
    )  # Assuming `read_image` uses cv2.imread or similar method
    pool = Pool(POOL_SIZE)
    all_boxes = []
    x_coords = []
    y_coords = []
...
def place_tile(img, box):
    ...
    if OVERLAP_TILES:
        empty_mask = (img_box[:, :, 3] == 0) & mask
        img_box[empty_mask] = box["tile"][: img_box.shape[0], : img_box.shape[1], :][
            empty_mask
        ]
    elif not np.any(img_box[mask]):
...
def find_most_common_scale(tile_keys):
    ...
    for size1, size2 in itertools.combinations(tile_keys, 2):
        ...
        if h2 % h1 == 0 and w2 % w1 == 0:
            scale_count[size1] += 1
            scale_count[size2] += 1
            size_groups[size1].add(size2)
            divisible_by[size2].add(size1)
        elif h1 % h2 == 0 and w1 % w2 == 0:
            scale_count[size1] += 1
            scale_count[size2] += 1
            size_groups[size2].add(size1)
            divisible_by[size1].add(size2)
    ...
    return most_common_sizes, non_multiple_sizes, size_groups, divisible_by
...
def main():
    ...
    tiles_paths = ["./tiles/lego/gen_lego_h"]
